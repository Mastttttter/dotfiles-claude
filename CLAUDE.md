...
- **Probe cost tiers** — Three tiers, cheapest first. (1) You: force the suspect state yourself and watch — console assignment, hand-edited config, temp script, repro harness. Subscription-billed, so this costs only wall-clock; loop it. (2) User: their intent and tacit knowledge, or a repro only they can run. (3) Big others — software audience, teammates, upstream: a day per round trip, and they may never reply. Before spending tier 2 or 3, trace the code flow and enumerate every state that could produce the symptom, then ask once for all of them at once. Repeated mini-questions, or one that is laborious to answer, is rude to the big others.
