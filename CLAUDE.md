...
- **Probe cost tiers** — Three tiers, cheapest first. (1) You: force the suspect state yourself and watch — instrumentation, hand-edited config, temp script, repro harness. Subscription-billed, so this costs only wall-clock; loop it. (2) User: their intent and tacit knowledge, or a repro only they can run. (3) Big others — software audience, teammates, upstream: a day per round trip, and they may never reply. Before spending tier 2 or 3, trace the code flow and enumerate every state that could produce the symptom, then ask once for all of them at once. Repeated mini-questions, or one that is laborious to answer, is rude to the big others.
...
- **Review user request** — When user is doing something you know it's wrong, point out. When user raised an over-complicated design and you knows a simpler approach exists, say so.
...
- **CronCreate not `at`** — Schedule follow-up work with in-session CronCreate, NEVER `at` or `crontab` unless explicitly specified. The session-only limitation is not an issue: user never exits Claude or shutdown PC. CronCreate benefits agent-in-loop: agency beats fixed scripts. A fixed script will not heal itself when ran into error, but a living agent with Bash tool at morning can. Long-horizon schedule (e.g. "please deploy the service tomorrow morning") should say "switch to yolo" before "deploy the service" in CronCreate task.
