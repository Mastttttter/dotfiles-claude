...
- For non-Bash hooks (markers belong in tool-specific JSON fields, not a Bash command), inline a location-specific bypass instruction in `emit_pre_tool_deny` directly. Don't call `emit_pre_tool_deny_bypassable` — its footer references "the Bash command", which would be wrong for those tools.
